...
const CLAIM_TYPES = ["variation_change", "delay_eot", "payment_dispute", "disruption", "acceleration", "backcharge"];
...
export async function createClaim(input: {
  ...
  if (error || !claim) return { error: error?.message ?? "Could not save the claim." };

  const ids = (input.event_ids ?? []).filter(Boolean);
  if (ids.length) {
    await supabase.from("claim_events").insert(ids.map((event_id) => ({ claim_id: claim.id, event_id })));
  }

  revalidatePath("/claims"); revalidatePath("/");
  return { id: claim.id };
}